...
hyperParms <- list(
  augmentation = "none",
  netDefSummary = "All Input -> Mystery Hidden Layer -> Magic Hidden Layer -> Output",
  numIterations = 10000,
  miniBatchSize = 1,
  acceleration = "sse"
)
...
#form <- total_amount ~ trip_distance+RatecodeID+DryBulbTemp;
...
BEGIN	
	...
	set @predictScript = N'
	   library("MicrosoftML");
	   print(summary(greenBigAmountTest));
