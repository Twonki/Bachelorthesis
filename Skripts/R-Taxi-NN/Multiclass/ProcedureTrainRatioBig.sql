...
-- =============================================
DROP PROCEDURE IF EXISTS [dbo].[TrainRatioBigNN];
GO
CREATE PROCEDURE [dbo].[TrainRatioBigNN] 
@TrainingSize BigInt	
AS
BEGIN
	...
	set @cmd = N'
		...
		form <- RatecodeID ~ trip_distance+total_amount+duration_in_minutes+passenger_count+PULocationID+DOLocationID+extra+mta_tax+fare_amount
	...
	insert into Models (timest,model,model_name)
		select CURRENT_TIMESTAMP as timest, model, 'NNRatioBig' as name from #m;  
	drop table #m
END