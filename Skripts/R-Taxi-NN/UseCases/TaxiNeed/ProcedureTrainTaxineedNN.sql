--=========================================================================================
-- Creates NN to estimate the Rides
-- This is Usecase Rides #6
...
-- Output: A Linear Regression Model to predict Rides
--=========================================================================================
USE Taxi2Bachelor;
GO

DROP PROCEDURE IF EXISTS [dbo].[TrainRidesNN];
GO
CREATE PROCEDURE [dbo].[TrainRidesNN] 
@TrainingSize BigInt	
AS
BEGIN
	...
	INSERT INTO #TmpData
		Select 
			Count(total_amount)as trainRides
	...
	set @cmd = N'
		...
		netDefinition <- ("
			input Data auto;
			hidden Mystery [300] tanh from Data all;
		...
		#Formel
		form <-  trainRides ~ trainDate+trainTempLevel+trainLocation+trainDayTime
	...
	INSERT INTO Models (timest,model,model_name)
		SELECT CURRENT_TIMESTAMP AS timest, model, 'NNRides' AS name FROM #m;  
	DROP TABLE #m
	DROP TABLE IF EXISTS #TmpData;
END
GO