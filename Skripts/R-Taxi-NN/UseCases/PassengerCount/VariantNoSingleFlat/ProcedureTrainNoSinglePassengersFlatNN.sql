...
-- Variante: No Deep Learning, only one hidden Layer with tanh 
--=========================================================================================
USE Taxi2Bachelor;
GO
DROP PROCEDURE IF EXISTS [dbo].[TrainNoSinglePassengersFlatNN];
GO
CREATE PROCEDURE [dbo].[TrainNoSinglePassengersFlatNN] 
@TrainingSize BigInt	
AS
BEGIN
	--=====================
	...
	set @cmd = N'
		...
		netDefinition <- ("
			input Data auto;
			hidden Mystery [1000] linear from Data all;
			output Result auto from Mystery all;
		")
	...
	INSERT INTO Models (timest,model,model_name)
		SELECT CURRENT_TIMESTAMP AS timest, model, 'NNNoSinglePassengersFlat' AS name FROM #m;  
	DROP TABLE #m
