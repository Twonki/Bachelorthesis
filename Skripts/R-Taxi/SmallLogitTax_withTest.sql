...
GO
UPDATE tip_models
SET model_name = ('rxSmallLogit ' + format(getdate(), 'yyyy.MM.HH.mm', 'en-gb'))
...
AS
BEGIN
	DECLARE @tipmodel varbinary(max) = (SELECT TOP(1) model FROM tip_models WHERE (model_name LIKE 'rxSmallLogit%'));
	EXEC sp_execute_external_script
		@language = N'R'
		, @script = N'
