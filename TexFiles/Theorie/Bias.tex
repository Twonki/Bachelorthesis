...
Fehler aufgrund von Verzerrung und Fehler aufgrund von [natürlicher] Varianz. Es gibt einen Zusammenhang zwischen der Fähigkeit eines Modells, die Abweichung und die Varianz zu minimieren. Diese beiden Fehler zu verstehen, hilft die Ergebnisse des Modells auszuwerten und die Fehler für \textit{Under-} und \textit{Overfitting} zu vermeiden. (\cite{BiasVarianceDilemma} Vorwort, Übersetzt).
...
~\newline In diese Abschnitt wird zuerst das Bias-Variance-Dilemma vorgestellt und anschließend kurz verschiedene Formen von \textit{Bias} (hier dt. Varianz). 
